...
describe('Reporting-owned SearXNG deployment', () => {
  ...
  it('health-checks the local process and proxy without performing an external search', () => {
    ...
    expect(test).toContain('host.docker.internal')
    expect(test).toContain('8118')
    expect(test).not.toMatch(/\/search|q=/)
  })

  it('routes engine traffic to host Privoxy without joining its shared Docker network', () => {
    expect(compose.services.searxng.networks).toEqual(['default'])
    expect(compose.services.searxng.extra_hosts).toEqual(['host.docker.internal:host-gateway'])
    expect(compose.networks?.proxy).toBeUndefined()
    expect(settings.outgoing.proxies['all://']).toEqual(['http://host.docker.internal:8118'])
    expect(JSON.stringify(compose)).not.toContain('vpnserver-proxy_default')
  })
