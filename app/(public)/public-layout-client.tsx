...
import { publicRootSurface } from '@/lib/platform-landing/public-root'
...
export default function PublicLayoutClient({
  ...
  readonly hostContext: FundHostContext
}) {
  const router = useRouter()
  const pathname = usePathname()
  const surface = publicRootSurface(hostContext, pathname)
  const [authChecked, setAuthChecked] = useState(false)

  useEffect(() => startLegacyPublicAuthCheck({
    surface,
    getUser: () => createClient().auth.getUser(),
    replace: destination => router.replace(destination),
    revealPublicShell: () => setAuthChecked(true),
  }), [surface, router])
