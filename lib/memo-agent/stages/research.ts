...
export async function runResearch(params: {
  ...
  const [claimsResult, competitorsResult, foundersResult] = await Promise.all([
    ...
    runSubCall({
      ...
      maxTokens: 8192,
      parse: (obj) => parseFounderDossiers(obj.founder_dossiers),
  ...
  if (params.persist !== false) {
    ...
    const { error: updateErr } = await admin
      ...
      .update({ research_output: output as unknown as Json })
      .eq('id', draftRow.id)
      .eq('deal_id', dealId)
      .eq('fund_id', fundId)
      .eq('is_draft', true)
...
function parseFounderDossiers(value: unknown): ResearchOutput['founder_dossiers'] {
  return parseModelList(value, item => {
