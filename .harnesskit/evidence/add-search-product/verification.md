...
- Merge security follow-up: SearXNG no longer joins the shared proxy Docker network; it stays on `reporting-search_default` and reaches host-published Privoxy port `8118` through an explicit `host-gateway` mapping. Compose validation and all five deployment contract tests pass. The live container was recreated using its existing secret, verified with the host-gateway entry, and reached healthy status.
