...
The deployment routes SearXNG engine traffic through the existing host-published Privoxy service at port `8118`. SearXNG remains only on its Compose-private default network and reaches Privoxy through the explicit Linux `host-gateway` mapping. It never joins the proxy's shared Docker network, so unrelated containers on that network cannot bypass Reporting and call SearXNG's internal port directly. The healthcheck verifies both the local SearXNG process and proxy reachability without executing an external search, while the 4-second engine timeout remains below Reporting's 8-second adapter deadline.

The host proxy remains an operator-owned deployment boundary and must be firewalled appropriately. Production may replace the host-gateway path with an equivalently isolated forwarder, but must not attach the unauthenticated SearXNG service to a shared application network.
